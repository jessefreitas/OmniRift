# Orquestrador Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** dar aos agentes do OmniRift um canal de comunicação ativo peer-a-peer — perguntar, avisar e negociar — sobre a infra de orquestração que já existe, unificado sob o nome Orquestrador.

**Architecture:** 3 tools MCP novas (`agent_status`/`agent_ask`/`agent_tell`) no servidor MCP embutido (`mcp/server.rs` + `mcp/tools.rs`), sobre um **protocolo de marcador** (`[[OMNIRIFT-ASK/REPLY/MSG]]`) injetado no PTY e casado por `uuid` no read-loop. Um **preâmbulo de role** (via `agent_mcp_config`) ensina todo agente a responder o protocolo e a negociar claims. Camadas seguintes (monitor, barramento, FS guard) reusam esse núcleo.

**Tech Stack:** Rust (Tauri 2, tokio, axum, dashmap), `mcp/` (JSON-RPC 2.0 + SSE), `pty/` (portable-pty + VT100 screen), front React/TS.

**Referência viva:** spec `docs/superpowers/specs/2026-07-09-orquestracao-design.md` (as 7 camadas). Este plano detalha **Fase 4a** (núcleo pull) a granularidade executável; **4b/7/5/6** vêm como tarefas concretas que referenciam as funções definidas na 4a (refinadas a bite-sized quando 4a aterelar as interfaces).

---

## Camadas 1–3 — já existem (sem tarefas, contrato de referência)

Não há trabalho novo aqui; o Orquestrador consome o que existe:
...
- **Create** `apps/desktop/src-tauri/src/mcp/marker.rs` — tipos + parser puro dos marcadores Orquestrador (unit-testável sem PTY).
...
- **Modify** `apps/desktop/src-tauri/src/mcp/server.rs` — helpers internos `orq_ask_and_wait()` e `orq_deliver_msg()` (reusam padrão do `do_send_task`); export pra `tools.rs`.
- **Modify** `apps/desktop/src-tauri/src/commands/mcp.rs` — `agent_mcp_config` injeta o preâmbulo de role Orquestrador.
...
```rust
//! Protocolo de marcador Orquestrador: linhas especiais no PTY que os agentes trocam.
...
git commit -m "feat(orquestração): parser puro dos marcadores ASK/REPLY/MSG"
```

---

### Task 2: `orq_ask_and_wait` — injeta ASK, bloqueia até REPLY
...
async fn ask_and_wait_times_out_with_state_hint() {
    ...
    let out = orq_ask_and_wait(&st, "inexistente", "@A", "oi?", 1).await;
...
Expected: FAIL — `cannot find function orq_ask_and_wait`.
...
git commit -m "feat(orquestração): orq_ask_and_wait — inject ASK, bloqueia até REPLY correlacionado"
```

---

### Task 3: `orq_deliver_msg` — injeta MSG fire-and-forget
...
async fn deliver_msg_errors_on_unknown_target() {
    let st = test_state();
    let out = orq_deliver_msg(&st, "inexistente", "@A", "aviso").await;
...
pub async fn orq_deliver_msg(state: &McpState, target_label: &str, from: &str, message: &str) -> String {
    let sid = match state.agent_registry.get_session_id(target_label) {
        Some(s) => s,
        None => return format!("❌ Agente '{target_label}' não encontrado."),
    };
...
```bash
git add apps/desktop/src-tauri/src/mcp/server.rs
git commit -m "feat(orquestração): orq_deliver_msg — MSG fire-and-forget (primitivo do barramento)"
```
...
git commit -m "feat(orquestração): agent_status (peek barato) + schemas de agent_ask/agent_tell"
```
...
> **Decisão de wiring:** `agent_ask`/`agent_tell` são `async` e chamam funções de `server.rs`. Se o `match tool` de `tools.rs` já é `async` (o `orchestration_send`/`terminal_send_text` usam `.await`), fiar lá. Se `orq_ask_and_wait` não for visível de `tools.rs`, rotear em `server.rs::handle_jsonrpc` (onde `do_send_task` já vive), como já se faz com `review_current`.
...
"agent_ask" => {
    ...
    let text = orq_ask_and_wait(&state, target, from, question, timeout_s).await;
...
"agent_tell" => {
    ...
    let text = orq_deliver_msg(&state, target, from, message).await;
...
git commit -m "feat(orquestração): wire agent_ask/agent_tell nos helpers de entrega"
```

---

### Task 6: preâmbulo de role Orquestrador (agentes nascem cientes do protocolo)
...
fn preamble_contains_reply_protocol_and_claim_etiquette() {
    let p = orquestração_preamble();
    assert!(p.contains("OMNIRIFT-ASK"));
    assert!(p.contains("OMNIRIFT-REPLY"));
...
Expected: FAIL — `orquestração_preamble` não existe.

- [ ] **Step 3: Write minimal implementation**

Em `commands/mcp.rs`:
```rust
/// Etiqueta injetada no system prompt de todo agente Orquestrador.
pub fn orquestração_preamble() -> &'static str {
    "Você participa do Orquestrador. Outros agentes falam com você:\n\
...
```
E incorporar o retorno de `orquestração_preamble()` na string de instrução/append que `agent_mcp_config` já monta pro spawn (concatenar ao bloco de role, sem quebrar o merge de MCP existente).
...
git commit -m "feat(orquestração): preâmbulo de role — agentes nascem cientes do protocolo + negociação"
```
...
git commit -m "feat(orquestração): from real via X-Terminal-ID (quem pergunta é identificado)"
```
...
**Files:**
- Create: `apps/desktop/src-tauri/tests/orquestração_e2e.rs` (ou `#[ignore]` no módulo, rodado sob demanda)
...
Run: `cd apps/desktop/src-tauri && cargo test -p omnirift --test orquestração_e2e -- --ignored 2>&1 | tail -30`
...
```bash
git add apps/desktop/src-tauri/tests/orquestração_e2e.rs
git commit -m "test(orquestração): e2e ask→reply correlacionado entre 2 agentes (ignored no CI)"
```
...
- [ ] **Step 4: Commit** (se algo precisou de ajuste): `git commit -am "chore(orquestração): fase 4a — regression guard verde"`.

---

## FASE 4b — Push dirigido + negociação (após 4a verde)

- **Task 10:** `agent_tell` já existe (Task 3+5); adicionar teste de integração do push (B faz `agent_tell(A, ...)`, A vê o MSG na tela no próximo `read_screen`). Files: `tests/orquestração_e2e.rs`.
- **Task 11:** fluxo de negociação e2e — A faz `claim_acquire(x)`; B faz `claim_check(x)` → recebe "travado por A" → `agent_ask(A, "libera x?")` → A responde → assert sem colisão. Files: `tests/orquestração_e2e.rs`. Reusa `orq_ask_and_wait` (Task 2) + `claim_*` (existe). Sem código novo de produção — valida a etiqueta do preâmbulo (Task 6).
- **Task 12:** filtrar os marcadores `[[OMNIRIFT-*]]` do que o xterm renderiza pro humano (meta, não ruído visível). Files: `pty/session.rs` (read-loop) + front `TerminalNode.tsx` (esconder linhas-marcador). TDD no parser (marker.rs já isola o reconhecimento).
...
- **Task 15:** dispatcher de tópicos sobre `orq_deliver_msg` (Task 3): tabela `subscriptions(topic, agent)`; `orquestração_publish(topic, payload)` injeta MSG em cada inscrito. Files: `mcp/tools.rs` (tools `topic_subscribe`/`topic_publish`) + `mcp/server.rs`. Reusa 100% o primitivo da Task 3.
...
- **Type consistency:** `orq_ask_and_wait`/`orq_deliver_msg`/`orquestração_preamble`/`from_label`/`render_ask`/`render_msg`/`parse_reply`/`find_reply` — nomes usados de forma idêntica onde referenciados. Tools: `agent_status`/`agent_ask`/`agent_tell` consistentes em schema e handler.
