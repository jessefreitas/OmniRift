  ...
  const card = (
    <>
      ...
      <input
        ...
        placeholder="tar -xzvf bundle.tar.gz | grep -i erro"
      ...
      <div className="shrink-0 flex flex-wrap items-center gap-1 px-2 py-2 border-b border-border nodrag" onPointerDown={(e) => e.stopPropagation()}>
        {segments.length === 0 ? (
          <span className="text-[10px] text-textMuted opacity-50">Digite um comando acima.</span>
        ) : (
          segments.map((s, i) => (
      ...
      <div className="flex-1 overflow-auto bg-bg nodrag" onPointerDown={(e) => e.stopPropagation()}>
        {segments.map((s, i) => {
          const desc =
            s.kind === "command"
              ? (whatis[s.command ?? ""] || "comando / binário executado")
              : s.explanation;
          return (
