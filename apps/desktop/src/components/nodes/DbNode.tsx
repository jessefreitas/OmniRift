...
export function DbNode({ id, data, selected }: NodeProps<DbRfNode>) {
  ...
  const resultPane = error ? (
    ...
    <p className="px-2 py-2 text-[10px] text-textMuted opacity-50">
      Clique numa tabela à esquerda, ou escreva uma query e rode (Ctrl+Enter).
    </p>
  );
  ...
  const card = (
    <>
      ...
      <div className="flex items-center gap-1 px-2 py-1.5 border-b border-border nodrag">
        <button onClick={() => void pickFile()} onPointerDown={(e) => e.stopPropagation()} title="Abrir arquivo .sqlite" className="shrink-0 flex items-center gap-1 px-2 py-1 rounded text-[11px] bg-surface2 text-text hover:bg-bg border border-border transition-colors">
          <FolderOpen size={11} /> Abrir
        </button>
        <input value={dbPath} onChange={(e) => setDbPath(e.target.value)} onBlur={() => patchNode(id, { dbPath })} onPointerDown={(e) => e.stopPropagation()} placeholder="/caminho/para/app.db" className="flex-1 min-w-0 px-1.5 py-1 rounded text-[11px] bg-bg border border-border text-text placeholder:text-textMuted focus:outline-none focus:border-brand font-mono" />
      </div>
      ...
      <div className="flex-1 flex min-h-0">
        <div className="w-40 shrink-0 border-r border-border bg-surface1 overflow-auto nodrag" onPointerDown={(e) => e.stopPropagation()}>
          <div className="px-2 py-1 text-[9px] uppercase tracking-wider text-textMuted border-b border-border sticky top-0 bg-surface1">Tabelas ({tables.length})</div>
          {tables.length === 0 ? (
            <p className="px-2 py-2 text-[10px] text-textMuted opacity-50">{dbPath ? "sem tabelas" : "abra um .db"}</p>
          ) : (
            tables.map((t) => (
        ...
        <div className="flex-1 flex flex-col min-w-0">
          <div className="relative shrink-0 border-b border-border">
            ...
            />
            <button onClick={() => void runSql()} disabled={loading || !dbPath.trim() || !sql.trim()} title="Rodar (Ctrl+Enter)" className="absolute bottom-1.5 right-1.5 flex items-center gap-1 px-2 py-1 rounded text-[11px] bg-brand text-bg hover:bg-brand-hover disabled:opacity-40 disabled:cursor-not-allowed transition-colors">
