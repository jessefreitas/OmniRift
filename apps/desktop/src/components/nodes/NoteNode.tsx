  ...
  return (
    ...
    >
      ...
      <div className="node-drag-handle flex items-center gap-1.5 px-2 py-1 cursor-grab active:cursor-grabbing bg-black/5 select-none">
        ...
        <NodeHelp side="bottom" className="text-black/40 hover:text-black/80" text={t("note.help", "Nota adesiva: escreva texto livre. O alfinete (📌) salva o texto nos Lembretes com prazo. O círculo troca a cor; arraste pelo topo.")} />
        <button
          onClick={(e) => { e.stopPropagation(); void saveReminder(); }}
          title={saved ? t("note.saved", "Salvo nos Lembretes") : t("note.saveReminder", "Salvar como lembrete")}
        ...
        <button
          ...
          title={t("note.changeColor", "Trocar cor")}
          className="w-3 h-3 rounded-full border border-black/20 shrink-0"
          style={{ background: nextColor }}
        />
        <button
          onClick={(e) => { e.stopPropagation(); removeNode(id); }}
          title={t("note.delete", "Excluir nota")}
