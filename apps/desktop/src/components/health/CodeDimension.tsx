...
export function CodeDimension({ files, summary, scanning }: Props) {
  ...
  async function analyze(paths: string[]) {
    for (const path of paths) {
      ...
      try {
        const report = await healthAnalyzeFile(path);
              ...
              <div key={f.path}>
                <div className="px-2 py-1 grid grid-cols-[auto_1fr_auto_auto_auto] gap-2 items-center rounded-md hover:bg-surface1 group">
                  ...
                  <button
                    ...
                    title={t("health.analyzeFile", "analisar IA")}
                    className="col-start-2 col-span-4 justify-self-end -mt-1 hidden group-hover:flex items-center gap-1 text-[10px] text-brand hover:underline disabled:opacity-50"
                  >
                    {isAnalyzing ? <Loader2 size={11} className="animate-spin" /> : <Sparkles size={11} />}
                    {isAnalyzing ? t("health.analyzing", "analisando…") : t("health.analyzeFile", "analisar IA")}
                  </button>
                </div>
                {errors[f.path] && (
