    ...
    <div className="fixed inset-0 z-[9999] flex items-center justify-center bg-black/50 p-4" onClick={onClose}>
      ...
      >
        <header className="flex items-center gap-2 px-4 py-2.5 border-b border-border shrink-0">
          ...
          <span className="text-sm font-medium text-text">{t("compressors.title", "Compressores de Token")}</span>
          <div className="flex-1" />
          <button onClick={() => setAdding((a) => !a)} title={t("compressors.addCustomTitle", "Adicionar compressor personalizado")} className="text-textMuted hover:text-brand p-1">
            <Plus size={15} />
          </button>
          <button onClick={refresh} title={t("compressors.redetect", "Re-detectar")} className="text-textMuted hover:text-brand p-1">
        ...
        {adding && (
          <div className="px-4 py-3 border-b border-border bg-surface2 space-y-1.5">
            <div className="text-[10px] uppercase tracking-wide text-textMuted">{t("compressors.customCompressor", "Compressor personalizado")}</div>
            <input
              value={newComp.label}
              onChange={(e) => setNewComp((s) => ({ ...s, label: e.target.value }))}
              placeholder={t("compressors.namePlaceholder", "Nome (ex: MeuCompressor)")}
            ...
            <input
              ...
              placeholder={t("compressors.installCmdPlaceholder", "Comando de instalação (ex: pip install … / cargo install --git …)")}
            ...
            <div className="flex items-center justify-end gap-1.5 pt-0.5">
              <button onClick={() => { setAdding(false); setNewComp({ label: "", installCmd: "" }); }} className="px-2 py-1 text-[11px] text-textMuted hover:text-text">{t("common.cancel", "Cancelar")}</button>
              <button onClick={saveNewComp} disabled={!newComp.label.trim() || !newComp.installCmd.trim()} className="px-2 py-1 text-[11px] rounded bg-brand text-bg hover:bg-brand-hover disabled:opacity-40">{t("common.add", "Adicionar")}</button>
            </div>
          </div>
        )}
        ...
        <div className="flex-1 overflow-auto">
          ...
          {custom.map((c) => (
            <div key={c.id} className="flex items-start gap-3 px-4 py-3 border-b border-border/40">
              <div className="min-w-0 flex-1">
                <div className="flex items-center gap-2">
                  <span className="text-sm text-text font-medium">{c.label}</span>
                  <span className="text-[9px] uppercase tracking-wide px-1 py-0.5 rounded bg-surface2 text-textMuted">{t("compressors.customBadge", "personalizado")}</span>
                </div>
              ...
              <button onClick={() => removeCustom(c.id)} title={t("common.remove", "Remover")} className="shrink-0 text-textMuted hover:text-danger px-1 py-1.5">
                <Trash2 size={14} />
              </button>
            </div>
          ))}
          ...
          {!loading && list.length === 0 && custom.length === 0 && (
            <p className="px-4 py-4 text-[12px] text-textMuted">{t("compressors.empty", "Nenhum compressor. Use o + pra adicionar um personalizado.")}</p>
          )}
        </div>
