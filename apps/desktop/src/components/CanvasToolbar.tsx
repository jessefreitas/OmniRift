...
import { Activity, BarChart3, Braces, Database, FileCode2, FileText, FolderTree, Frame, Globe, Pencil, ScrollText, StickyNote, TerminalSquare, Webhook, Wrench, Zap } from "lucide-react";
...
export function CanvasToolbar() {
  ...
  return (
    <div className="absolute top-3 left-1/2 -translate-x-1/2 z-30 flex items-center gap-0.5 px-1.5 py-1 rounded-xl bg-surface2/90 backdrop-blur border border-border shadow-lg">
      ...
      />
      <ToolBtn
        label={currentCwd ? t("toolbar.codeMetrics", "Complexidade do projeto") : t("toolbar.openProjectFirst", "Abra um projeto primeiro")}
        icon={BarChart3}
        disabled={!currentCwd}
        onClick={() => currentCwd && window.dispatchEvent(new CustomEvent("omnirift:open-tool", { detail: "code-metrics" }))}
      />
      <ToolBtn
