...
export async function analyzeCanvas(): Promise<string> {
  ...
  if (!cfg) {
    return "Configure um LLM em Ferramentas → 'LLM do review (BYOK)'. O Companheiro usa o mesmo (qualquer provider).";
  }
  ...
  const system =
    "Você é o Companheiro do OmniRift: observa o canvas de agentes de IA e ajuda o humano a orquestrá-los. Seja conciso e acionável, em PT-BR.";
  const prompt =
