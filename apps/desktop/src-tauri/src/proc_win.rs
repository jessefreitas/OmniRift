...
#[cfg_attr(not(windows), allow(dead_code))]
pub(crate) fn resolve_program_portable(
    command: &str,
    path_dirs: &[String],
    pathext: &[String],
...
pub(crate) fn resolve_windows_program(command: &str) -> Option<ResolvedProgram> {
    let dirs: Vec<String> = crate::pty::session::effective_path_parts()
        .iter()
    ...
    let pathext: Vec<String> = pathext_env
        .split(crate::pty::session::PATH_SEP)
        .map(str::trim)
        .filter(|s| !s.is_empty())
        .map(|s| {
            if s.starts_with('.') {
...
///
/// Agora: `resolve_windows_program` acha o alvo pelo PATH+PATHEXT e devolvemos o caminho
/// ABSOLUTO. Um `.exe` spawna direto. Um shim `.cmd`/`.bat` também pode ir direto porque
/// o `std` trata batch como caso especial e monta a linha com as regras do `cmd` — é o
/// endurecimento do Rust 1.77.2 (CVE-2024-24576), e 1.77.2 é justamente o `rust-version`
/// deste crate. Fora do Windows continua sendo no-op.
///
/// Se a resolução falhar (PATH diferente, binário ausente), cai no `cmd.exe /d /c` com os
/// tokens SEPARADOS — nunca mais numa linha única pré-quotada. `/d` pula o AutoRun do
/// registro, que injetaria saída de terceiro no stdio do adapter.
pub fn wrap_for_windows(program: &str, args: &[String]) -> (String, Vec<String>) {
    #[cfg(windows)]
    {
        ...
        plan_windows_spawn(program, args, resolve_windows_program(program), &comspec)
    }
    #[cfg(not(windows))]
    {
