...
mod tests {
    ...
    fn mobile_allows_the_readonly_9_methods() {
        ...
        for m in ["permissions.list", "kanban.list"] {
            assert!(
                is_allowed(m, DeviceScope::Mobile),
                "{m} (read-only #9) deve ser permitido"
            );
        }
        ...
        for m in ["permission.respond", "kanban.move"] {
            assert!(
                !is_allowed(m, DeviceScope::Mobile),
                "MUTAÇÃO '{m}' não pode ser read-only"
            );
        }
    }
    ...
    fn mobile_forbids_the_phase2_write_methods() {
        ...
        for m in ["agent.spawn", "agent.send", "agent.kill"] {
            ...
            assert!(
                is_allowed(m, DeviceScope::Runtime),
                "'{m}' deve rodar via socket local"
            );
        }
        // Belt-and-suspenders: a constante literalmente não contém nenhum dos 3.
        for m in ["agent.spawn", "agent.send", "agent.kill"] {
            assert!(
                !MOBILE_RPC_METHOD_ALLOWLIST.contains(&m),
                "'{m}' vazou pra allowlist!"
            );
        }
    }
    ...
    fn allowlist_is_exactly_the_mvp_set() {
        ...
        assert_eq!(
            MOBILE_RPC_METHOD_ALLOWLIST.len(),
            8,
            "read-only mobile = 8 métodos"
        );
    }
    ...
    fn steer_allowlist_is_exactly_the_three_mutations() {
        ...
        assert_eq!(
            MOBILE_STEER_ALLOWLIST.len(),
            7,
            "steering = 3 agente + 2 #9 + 2 ACP"
        );
        for m in [
            "agent.spawn",
            "agent.send",
    ...
    fn steer_does_not_unlock_anything_else() {
        ...
        for m in [
            ...
            assert!(
                !is_steer_allowed(m),
                "'{m}' NÃO pode ser destravado por steering"
            );
        }
    }
    ...
    fn gate_mobile_without_steer_forbids_mutations() {
        ...
        for m in ["agent.spawn", "agent.send", "agent.kill"] {
            assert!(
                !composed_gate(m, DeviceScope::Mobile, false),
                "'{m}' forbidden sem steer"
            );
        }
