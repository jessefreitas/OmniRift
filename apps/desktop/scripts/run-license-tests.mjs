...
//
// Runner dos testes PUROS do normalizador de chave de licença (apps/desktop não tem vitest).
// Bundla src/lib/license-client.test.ts com esbuild, resolvendo o alias `@/` → src/ e
// shimando `@tauri-apps/api/core`. Executa o bundle com node.
// Uso: node scripts/run-license-tests.mjs
...
const tauriShim = resolve(here, ".tauri-license-shim.mjs");
writeFileSync(
  tauriShim,
...
const result = await build({
  entryPoints: [resolve(root, "src/lib/license-client.test.ts")],
  bundle: true,
  write: false,
  format: "esm",
  platform: "node",
...
const out = resolve(here, ".license-test.bundle.mjs");
