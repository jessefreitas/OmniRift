// scripts/run-speculative-compact-tests.mjs
//
// Runner dos testes PUROS do classificador de preguiça (apps/desktop não tem vitest).
// Bundla src/lib/speculative-compact.test.ts com esbuild, resolvendo o alias `@/` → src/ e
// shimando `@tauri-apps/api/core`. Executa o bundle com node.
// Uso: node scripts/run-speculative-compact-tests.mjs
...
const result = await build({
  entryPoints: [resolve(root, "src/lib/speculative-compact.test.ts")],
  bundle: true,
  write: false,
  format: "esm",
  platform: "node",
...
const out = resolve(here, ".speculative-compact-test.bundle.mjs");
